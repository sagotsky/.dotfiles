export PATH="$HOME/scripts/:$HOME/bin:$HOME/bin/bin/:$HOME/.cabal/bin:/usr/bin:/usr/local/sbin:/usr/local/bin:/usr/sbin:/sbin:/bin:/usr/games:/usr/X11R6/bin:$PATH"
...
## screen session name labels for prompt
if [ $STY ] ; then
...
# sudo active (assuming -n (ver 1.7+) supported)
...
#	|				                                            |       |					  |        |	
#	|				                                            |       |					  |        ----- back to default color after the prompt
#	|				                                            |       |					  -------------- the actual prompt that gets displayed.  for all this text it sure is minimal
#	|                                           			    |       ----------------------------------- set color depending on context
#	|				                                            ------------------------------------------- show a useful title in supported terms
#	|----------------------------------------------------------- make sure to execute this before displaying prompt.  otherwise color vars never change

## Source some configs (.local files don't go in cvs)
